...
function logError(error, endpoint, method) {
  if (error.status) {
    ...
    console.log(`API request ${method.toUpperCase()} ${endpoint} responded with ${summary}`);
  }
  else {
    console.log(`API request ${method.toUpperCase()} ${endpoint} failed with message "${error.message}"`);
  }
}
